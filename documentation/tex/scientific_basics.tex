...
% TODO: Koen: Spell and definition checking, if you have more definitions to add, add it
...
This function receives a number $n$ for the total amount of queens and a number $r \in \{1 .. n\}$ for the row and return a set of clauses $K$ where only one queen can be placed in row $r$.
\\[0.2cm]
...
This function receives a number $n$ for the total amount of queens and a number $c$ where $c \leq n$ for the column and returns a set of clauses $K$ where at least one queen is placed in the column $c$.
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm} $\texttt{atMostOneInUpperDiagonal}(k,\; n) := \texttt{atMostOne}(\{\; Q_{x,y} :\; x,y \in \{1\; ..\; n\}\; |\; x + y = k\; \})$
...
\\[0.2cm]
\hspace*{1.3cm} $\texttt{atMostOneInLowerDiagonal}(k,\; n) := \texttt{atMostOne}(\{\; Q_{x,y} :\; x,y \in \{1\; ..\; n\}\; |\; x - y = k\; \})$

\paragraph{queensClauses}
With the functions above, we finally can define the N-Queens problem as a propositional logic formula in form of a set of clauses $K$.
\\[0.2cm]
\hspace*{1.3cm} $\texttt{queensClauses}: \N \to K$
\\[0.2cm]
\hspace*{1.3cm} $\texttt{queensClauses}(n) := \bigcup\limits_{i=1}^{n}(\texttt{atMostOneInRow}(i, n)\; \cup\; \texttt{oneInColumn}(i, n))\; \cup\; \bigcup\limits_{i=3}^{2 * n - 1}\texttt{atMostOneInUpperDiagonal}(i, n) \cup\; \bigcup\limits_{i=-(n - 2)}^{n - 2}\texttt{atMostOneInLowerDiagonal}(i, n)$