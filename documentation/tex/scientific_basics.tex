...
The set of all literals is called $\mathcal{K}$.
...
There is M as set of propositional logic formulas. If there is an interpretation $I$ so that all formulas of the set M are satisfied and the formula $\mathcal{I}(f) = \texttt{True}$ for all propositional logic formulas f $\in$ M is given, so set M is satisfiable. If this is not the case, then set M is not satisfiable.
