...
The purpose of this work is to visualize the Davis Putman algorithm that solves the so-called \textit{N-Queens Problem}. 
...
The empty set in the set of clauses $K_2$ is interpreted as a falsum in the propositional logic, making it impossible to satisfy $K_2$. It is possible to satisfy smaller sets of less complex clauses by first glance, but it gets more difficult for larger and complexer sets of clauses. For these algorithms like the Davis Putman algorithm are used.
%TODO: rephrase following sentence:
...
A set of clauses $K$ contains the empty set and is therefore unsatisfiable
...
To simplify a set of clauses $K$ with $C_1 \cup \{\; p\; \}\, C_2 \cup \{\; \neg p\; \} \in K$ the cut rule can be used. A typical application of the cut rule is.
\\
\hspace*{1.3cm}
...
$K = \{\; \{\; l,\; p,\; \neg q \;\},\; \{\; l\; \}\; \} \cup M$
\\[0.2cm]
It is clear that the clause $\{\; l\; \}$ implifies the clause $\{\; l,\; p,\; \neg q \;\}$, because if $\{\; l\; \}$ is satisfyed, then $\{\; l,\; p,\; \neg q \;\}$ will be satisfyed too. That's because
\\
\hspace*{1.3cm}
...
\\[0.2cm]
%TODO: fix this
that receives a set of clauses $K$ and a literal $l$. It will simplify $K$ by subsuming $K$ with the unit clause $\{\; l\; \}$ and keep the unit clause itself:
\\
\hspace*{1.3cm}
