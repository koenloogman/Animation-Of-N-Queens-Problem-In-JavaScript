...
Because in JavaScript uses when comparing objects with each other their references, we will use Immutable.js as said in chapter \ref{sec:tecImmutable} to counter this problem when working with sets. Because we will have a set of clauses with those clauses being sets of literals, we would need to implement a deep compare ourselves if we were to use native JavaScript. That said Immutable.js solves the set comparison, but we still need to implement literals somehow. We decided to use simple strings for the literals with the restriction of the first character not being allowed to be an ``!'', because we will use that character to indicate that the literal is negated. The choice to use strings is based on the reason for using Immutable.js for the sets. Objects are compared by their references and Immutable.js does the same when adding elements to their structures, but their own immutable ones they compare by value. This means if we were to create an literal object we would need to come up with our own way of comparing them. Strings on the other hand are compared by length and character order by default.
